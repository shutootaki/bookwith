  ...
  return (
    <div className="flex items-center space-x-2">
      <button
        ...
        aria-label={isMuted ? t('podcast.audio_player.unmute') : t('podcast.audio_player.mute')}
        title={`${t('podcast.audio_player.mute_toggle')} (${PODCAST_KEYBOARD_SHORTCUTS.MUTE_TOGGLE})`}
      >
        ...
        <div className="flex-1">
          <input
            ...
            title={`${t('podcast.volume_control.adjust')} (${PODCAST_KEYBOARD_SHORTCUTS.VOLUME_UP}/${PODCAST_KEYBOARD_SHORTCUTS.VOLUME_DOWN})`}
          />
        </div>
      <span className="text-muted-foreground w-12 text-sm text-right">
        {volumePercentage}
      </span>
    </div>
  )
