...
class _FakeChatManager:
    ...
    def ensure_chat_exists(
        self, chat_id: str, sender_id: str, book_id: str | None, content: str
    ) -> None:
...
class _FakeMessageProcessor:
    ...
    def save_user_message(self, *args: Any, **kwargs: Any) -> None:  # noqa: ANN401
        self.saved_user.append((args, kwargs))

    def save_ai_message(self, *args: Any, **kwargs: Any) -> None:  # noqa: ANN401
...
class _FakeAIResponseGenerator:
    ...
    async def stream_ai_response(
        self, *, question: str, user_id: str, book_id: str | None
...
async def test_stream_completes_within_timeout_persists_full_response() -> None:
    ...
    usecase, processor = _build_usecase(
        chunks=["hello ", "world"], chunk_delay=0.001, timeout_seconds=2.0
    )
    received: list[str] = []
    async for chunk in usecase.execute(
        content="q", sender_id="user-1", chat_id="chat-1", book_id=None
    ):
...
async def test_stream_timeout_yields_marker_and_persists_partial() -> None:
    ...
    usecase, processor = _build_usecase(
        chunks=["a", "b", "c", "d", "e"], chunk_delay=0.4, timeout_seconds=0.5
    )
    received: list[str] = []
    async for chunk in usecase.execute(
        content="q", sender_id="user-1", chat_id="chat-1", book_id=None
    ...
    assert "[stream timed out" not in persisted, (
        "保存される本文にタイムアウト通知メッセージは含めない"
    )
...
async def test_stream_persists_zero_chunk_does_not_save() -> None:
    ...
    usecase, processor = _build_usecase(
        chunks=["delayed"], chunk_delay=2.0, timeout_seconds=0.1
    )
    received: list[str] = []
    async for chunk in usecase.execute(
        content="q", sender_id="user-1", chat_id="chat-1", book_id=None
