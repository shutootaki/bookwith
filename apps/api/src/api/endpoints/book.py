...
from src.services.book_service import add_book, all_books, get_all_covers
...
async def get_book_cover(
    ...
    try:
        ...
        if gcs_client.use_emulator:
            # エミュレーター環境では直接URLを返す
            return JSONResponse(content={"success": True, "url": book.cover_path})
        else:
            # 本番環境では署名付きURLを生成
...
async def get_book_file(
    ...
    try:
        ...
        if gcs_client.use_emulator:
            # エミュレーター環境では直接URLを返す
            return JSONResponse(content={"success": True, "url": book.file_path})
        else:
            # 本番環境では署名付きURLを生成
            # GCSのパスからバケット名とオブジェクト名を抽出
