...
from src.usecase.podcast.services.epub_extractor_service import Chapter

logger = logging.getLogger(__name__)


class ChapterSummarizerService:
    """Service for summarizing book chapters using Gemini"""

    def __init__(self, gemini_client: GeminiClient) -> None:
        self.gemini_client = gemini_client
        # Limit concurrent requests to 2 as Gemini API is prone to failure with many simultaneous calls
        self.max_concurrent_requests = 2

    async def summarize_chapters(self, chapters: list[Chapter], book_title: str) -> str:
    ...
    async def _summarize_single_chapter(self, chapter: Chapter) -> str:
        ...
        try:
            ...
            attempts: list[tuple[int, int]] = [
                (4000, 256),  # initial try
                (2000, 192),  # shorter text
                (1000, 128),  # fallback for stubborn LENGTH errors
            ]
    ...
    async def _create_book_summary(self, chapter_summaries: list[str], book_title: str) -> str:
        ...
        if len(chapter_summaries) <= CHUNK_SIZE:
            return await self.gemini_client.combine_summaries(chapter_summaries, book_title, max_output_tokens=800, temperature=0.3)
        ...
        for i in range(0, len(chapter_summaries), CHUNK_SIZE):
            ...
            try:
                ...
                partial = await self.gemini_client.summarize_text(truncated_prompt, max_output_tokens=512, temperature=0.3)
                partial_summaries.append(partial)
    ...
    async def create_quick_summary(self, chapters: list[Chapter], book_title: str, max_chapters: int = 5) -> str:
